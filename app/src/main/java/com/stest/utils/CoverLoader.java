...
/**
    ...
    public Bitmap loadThumbnail(String uri) {
        ...
        if (TextUtils.isEmpty(uri)) {
            ...
            if (bitmap == null) {
                bitmap = BitmapFactory.decodeResource(NetEasyApplication.getInstance().getResources(), R.drawable.default_cover);
                mThumbnailCache.put(KEY_NULL, bitmap);
            }
        } else {
            bitmap = mThumbnailCache.get(uri);
            if (bitmap == null) {
                bitmap = loadBitmap(uri, ScreenUtils.getScreenWidth(NetEasyApplication.getInstance()) / 10);
                if (bitmap == null) {
    ...
    public Bitmap loadBlur(String uri) {
        ...
        if (TextUtils.isEmpty(uri)) {
            ...
            if (bitmap == null) {
                bitmap = BitmapFactory.decodeResource(NetEasyApplication.getInstance().getResources(), R.drawable.fm_run_result_bg);
                mBlurCache.put(KEY_NULL, bitmap);
            }
        } else {
            bitmap = mBlurCache.get(uri);
            if (bitmap == null) {
                bitmap = loadBitmap(uri, ScreenUtils.getScreenWidth(NetEasyApplication.getInstance()) / 2);
                if (bitmap == null) {
    ...
    public Bitmap loadRound(String uri) {
        ...
        if (TextUtils.isEmpty(uri)) {
            ...
            if (bitmap == null) {
                bitmap = BitmapFactory.decodeResource(NetEasyApplication.getInstance().getResources(), R.drawable.placeholder_disk_play_song);
                bitmap = ImageUtils.resizeImage(bitmap, ScreenUtils.getScreenWidth(NetEasyApplication.getInstance()) / 2, ScreenUtils.getScreenWidth(NetEasyApplication.getInstance()) / 2);
